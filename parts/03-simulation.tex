...
\chapter{Testing \acs{BDI}-based \aclp{MAS} using \acl{DES}}
...
%
We then present in \Cref{sec:prototype} an open-source prototype that allows a \ac{BDI} agent specification
written in \jakta{}
...
before concluding and discussing future works in \Cref{sec:conclusion}.
...
%
We also report in \Cref{subsec:discrete-simulation}
...
%
Originally a philosophical concept~\cite{BratmanEtAl1987},
...
\label{subsec:discrete-simulation}
...
As discussed in \Cref{subsec:discrete-simulation},
...
%
Using strategy \relatedMiddleware{}, as we discuss in more detail in \Cref{sec:bdi-over-des},
...
as we will discuss in \Cref{sec:bdi-over-des} and demonstrate in \Cref{sec:evaluation}.

\section{Integrating \ac{BDI} and \ac{DES}}
\label{sec:bdi-over-des}
...
In this section
(and, more specifically, in \Cref{sec:portable-agent-specifications}),
...
%
In particular,
as mentioned in \Cref{subsec:discrete-simulation},
...
\footnotetext{
    ...
    our assumptions discussed at the beginning of \Cref{sec:bdi-over-des} still apply.
    %
    In fact,
...
\label{sec:portable-agent-specifications}
...
both in simulation and in real-world deployments.
%
In (\Cref{ssec:design-simulated-environment})
...
\label{ssec:design-simulated-environment}

Having defined the requirements for portable \ac{BDI} agent specifications (\Cref{sec:portable-agent-specifications})
...
the scheduling of \emph{internal} events to \Cref{ssec:granularity}.
...
\subsubsection{\acf{AMA}}\label{subsubsec:ama}
...
\subsubsection{\acf{ACLI}}\label{subsubsec:acli}
...
\subsubsection{\acf{ACLP}}\label{subsubsec:aclp}
...
\subsubsection{\ac{ABE}}\label{subsubsec:abe}
...
as we discuss in \Cref{ssec:requirements} and in \Cref{ssec:choices}.
%
...
but it also allows us to study the impact of the many simulation granularity levels (see \Cref{ssec:granularity})
on the validation of a \ac{BDI} system.
%
This is indeed the purpose of \Cref{sec:evaluation}.
...
and archived on Zenodo~\cite{zenodojakta}
...
\label{ssec:choices}
In line with the requirements in \Cref{ssec:requirements},
...
indeed,
it has been used in the past to simulate programmable tuple spaces~\cite{ZambonelliPMC2015},
biochemical systems~\cite{MontagnaSAC2012},
and aggregate programming languages~\cite{PianiniSAC2015,scafi}.
%
Complementarily,
...
networked tuple spaces\cite{ZambonelliPMC2015},
and aggregate programming via Protelis~\cite{PianiniSAC2015} and Scafi~\cite{scafi}.
...
As discussed in \Cref{ssec:granularity},
...
%
We implemented all those introduced in \Cref{ssec:granularity} but \ac{ABE} because,
...
\subsection{\ac{BDI} \acp{MAS} in \alchemist{}}\label{subsec:mapping}
...
%
As introduced in \Cref{ssec:choices},
...
(\texttt{Molecule}s) and associated \texttt{Concentration}s (c.f. \Cref{ssec:choices}),
...
In this section,
we exercise the prototype introduced in \Cref{sec:prototype}
...
\footnotetext{
    ...
    %
    However,
    as we discuss extensively in \Cref{subsec:discrete-simulation},
...
$f = \nicefrac{1}{T}$, cf. \Cref{subsec:mapping}.
%
...
each of them experiences a random start delay $\tau_0 \in [0, \nicefrac{1}{f}]$
    (or, equivalently, $\tau_0 \in [0, T]$ see \Cref{subsec:mapping}).
...
with different granularities, namely,
    we consider \ac{AMA}, \ac{ACLI}, and \ac{ACLP} (cf. \Cref{ssec:granularity}).
%
...
that hosts a single \jakta{} agent controlling the \ac{UAV} as described in \Cref{subsec:mapping}.
...
%
We refer the interested reader to the companion artefact~\cite{zenodojakta}
...
We investigate how the mapping granularity (cf. \Cref{ssec:granularity}) impacts the system's behaviour.
%
To do so, we use \ac{AMA} as baseline,
letting the entire \ac{MAS} run a full cycle every simulated second
(Dirac Comb distribution with frequency $f=1Hz \rightarrow T=1s$, cf. \Cref{subsec:mapping}),
...
as discussed in \Cref{sec:related-works},
...
The UAV scenario in \Cref{sec:evaluation} has not been tested on real \acp{UAV}.
%
...
We here comment regarding the sub-objectives listed in \Cref{sec:introduction}.
...
\begin{itemize}
    \item \textbf{\ac{AMA}} (\Cref{subsubsec:ama}):
    ...
    \item \textbf{\ac{ACLI}} (\Cref{subsubsec:acli}):
    ...
    \item \textbf{\ac{ACLP}} (\Cref{subsubsec:aclp}):
    ...
    \item \textbf{\ac{ABE}} (\Cref{subsubsec:abe}):
