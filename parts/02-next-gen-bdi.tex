...
Long before the current surge of interest in \ac{GenAI},
the \ac{BDI} model has worked as a paradigm for rational agents,
leading to the development of several \ac{BDI} agent-oriented programming architectures~\cite{RaoG95},
as well as agent programming languages based on them%
---most notably, \agentspeak{}~\cite{rao-agentspeak96}.
%
\ac{BDI} languages design rational agents
able to reason about complex and dynamic environments,
and proactively act upon them.
...
%
% \gcsidenote{This capoverse should be reviewed once we have a clearer idea of the value added by our approach.}
% Mechanisms for automatic plan construction have previously been explored within the broader \ac{AI} literature,
% with \ac{RL} and classical planning serving as prominent examples.
% %
% However,
% these approaches typically face two significant drawbacks:
% they have not yet been stably integrated into existing \agentspeak{}-inspired programming frameworks,
% and most notably,
% they still depend heavily on fixed, pre-defined action models,
% requiring substantial manual specification from developers.
%
Extending \ac{BDI} agents with planning capabilities has thus been extensively explored~\cite{MeneguzziS15}.

Prior approaches typically attempt to bridge first-principles planning techniques with \ac{BDI} architectures~\cite{SilvaSP09},
requiring a detailed model of the action space.
%
However,
those models%,
%usually expressed in terms of preconditions and effects,
are typically costly and often unavailable for highly-dynamic environments.
%
% Also,
% \emph{automatic} planning techniques mostly generate plans that are effective for \emph{specific} goals and conditions,
% and lack the abstraction and generality that human plans typically feature instead\missingref{}---%
% so that they are difficult to reuse.
% %
% As a result,
% the costly planning process needs to be repeated even for similar goals.

Recent advances in \ac{GenAI} methodologies promise to overcome such limitations
by offering more dynamic and flexible solutions.
%
\ac{GenAI} technologies such as \acp{LLM} provide unprecedented opportunities for empowering cognitive agents
with intelligent features involving the generation of cognitive abstractions
(such as beliefs, goals, plans)
as well as the ability to interact in natural language%
---just like human beings would do.

Along that line, we ground our exploration on the assumptions that:
%
\begin{inlinelist}
    \item \acp{LLM} exhibit planning capabilities~\cite{HuangLWCWL24};
    \item \acp{LLM} easily handle natural language,
    and have absorbed \emph{common sense} knowledge that can be leveraged in the planning process~\cite{llmoracles-kbs310};
    and, finally
...
%
Furthermore,
observing that \ac{BDI} developers often rely on mnemonic and semantically-relevant names akin to natural language
to represent cognitive abstractions,
we suggest that \acp{LLM} can be exploited to generate \ac{BDI} plans
by simply relying on the natural-language semantics of goals, actions, and plans,
rather than explicitly-defined preconditions and effects.

Hence,
in this paper,
we focus on the problem of augmenting \ac{BDI} agents with the ability to autonomously generate \emph{plans}
...
%
To that end, 
we analyse requirements for integrating \ac{GenAI} into the \ac{BDI} architecture, 
propose a conceptual framework extending \ac{BDI} for plan generation, 
and discuss preliminary results using a \jakta{}~\cite{DBLP:journals/sncs/BaiardiBCP24} prototype.
...
related works on planning in \ac{BDI} systems,
and \ac{GenAI} agents.
%
In \Cref{sec:approach},
we present our approach to the integration of a \ac{GenAI}-based \ac{PGP} into \agentspeak{} agents,
followed by some practical remarks in \Cref{sec:remarks}.
%
In \Cref{sec:experiments}, we present preliminary experiments,
and in \Cref{sec:conclusion} we conclude and outline future work.

%==========================================
%Il paragafo sotto secondo me va nella discussion
%==========================================

% Whereas much contemporary research prioritizes creating cognitive agents that directly leverage \ac{GenAI},
% -- namely, where agents \emph{are} generative systems themselves, such as \ac{LLM} --,
% our research adopts an alternative yet complementary perspective:
% augmenting the well-established \agentspeak{}-based \ac{BDI} agent architecture
% by embedding \ac{GenAI} where necessary,
% while preserving agents' ordinary operation.
% %
% Such approach aims at picking up the best of both worlds:
% %
% \begin{inlinelist}
%     \item the \ac{BDI} model's strong theoretical foundations,
%     established programming paradigms,
%     and inherent controllability, predictability, and explainability properties;
%     and
%     \item the generative and \ac{NLP} capabilities of \ac{GenAI} systems,
% \end{inlinelist}
% %
% Therefore,
% our primary research goal is to investigate and systematically evaluate methodologies
% enabling \ac{BDI} agents to autonomously acquire novel plans,
% significantly enhancing their operational autonomy and adaptive behaviour.



% Specifically,
% in this paper,
% we focus on the problem of augmenting \agentspeak{} agents with the ability to autonomously generate \emph{plans},
% exploiting \ac{GenAI} under the hood.
% %
% In other words,
% we aim at studying if,
% and how,
% \ac{GenAI} technologies can be embedded into the \ac{BDI} architecture,
% to let agents automatically figure out what to do in unseen situations. %,
% %generating plans accordingly.
% %
% Along this line,
% we analyse the many ways in which this integration can be modelled and implemented,
% with a particular focus on the impact on the agent's architecture and operation,
% as well as on the agent programmer(s) experience.

% Put simply,
% our research attempts to provide an answer to the following questions:
% %
% \begin{enumerate*}[label=\textbf{(RQ\arabic*)}]
%     \item \emph{when} and \emph{how} should \ac{BDI} agents exploit generative capabilities to \emph{autonomously} generate plans?
%     \item \emph{what} information should \ac{GenAI} be provided with to generate plans \emph{effectively}?
%     \item in \emph{which} way is the role of the agent programmer(s) changing when agents can autonomously figure out what to do?
% \end{enumerate*}
%
% Moreover,
% we propose concrete extensions and novel syntactic constructs for agent-oriented programming languages like \agentspeak{},
% specifically engineered to facilitate generative plan acquisition.
% %
% These linguistic enhancements are intended to offer developers practical tools and methodologies
% to easily leverage generative functionalities,
% thereby promoting broader adoption.
% %
% Ultimately,
% our approach aims to significantly advance \ac{BDI} agents' capabilities,
% facilitating the creation of more intelligent, flexible, and autonomous cognitive systems
% capable of sophisticated reasoning and dynamic adaptation in complex and uncertain environments.
% In trying to answer these questions,
% we propose both a conceptual framework
% -- put simply, we discuss admissible extension of the \agentspeak{} architecture, aimed at embedding \ac{GenAI} --
% and a practical, prototype implementation of our framework based on \jakta{}~\cite{DBLP:journals/sncs/BaiardiBCP24}.
% %
% Finally, we compare our approach with existing literature on plan generation in \ac{BDI} agents,
% there including classical \ac{AI} planning and \ac{RL} techniques.

\section{Background}\label{sec:background}

Here,
we recall the main notions behind \ac{BDI} agents, plan generation in \ac{BDI} agents, and \ac{GenAI} agents.

\subsection{\ac{BDI} Agents and their Plans}\label{sec:bdi-agents}

The \ac{BDI} model,
inspired by human cognitive processes~\cite{bratman1987intention},
and grounded upon a formal framework~\cite{bdilogic-jlc8} for modelling \emph{rational agents}' decision-making,
has been adopted for agent programming languages (e.g., \cite{BordiniHW2007})
as well as for simulating intelligent agents~\cite{HubnerB09} or human behaviour~\cite{AdamGaudou2016}.
%
\ac{BDI} systems represent a wide yet specific class of \acp{MAS},
where rational agents feature \emph{cognitive} abstractions such as beliefs, goals, and intentions.
...
\begin{inlinelist}
    ...
    which may evolve over time or generate other desires to pursue;
    \item commits to multiple concurrent \emph{intentions} to fulfil its desires;
    \item executes \emph{plans} (i.e., sequences of \emph{actions}) in  order to achieve its goals.
\end{inlinelist}
...
%
In the following,
we focus on the widely-adopted \agentspeak{}---%
so that we may use the terms \ac{BDI} agents and \agentspeak{} agents interchangeably in the following.

% \agentspeak{}~\cite{RaoG95} agents are animated by events,
% to which they respond by selecting \emph{plans} that guide their execution.
% %
% Events may correspond to the addition or removal of beliefs
% -- which, in turn, may be the result of perception or communication with other agents --
% or to the occurrence of internal events,
% such as the commitment of the agent to a new goal to pursue.
% %
% Unlike purely reactive systems,
% where external stimuli are directly mapped to actions,
% \ac{BDI} agents reason about events in the context of their current state
% and choose \emph{plans} accordingly,
% via a continuous \emph{deliberation} activity.
%
% The execution of plans can, in turn, generate new internal events,
% allowing the agent to progress through its reasoning cycle
% and operate autonomously.

\agentspeak{} agents perform so-called \emph{procedural reasoning}~\cite{IngrandGR1992}:
they are equipped by human developers with procedural knowledge in terms of a \emph{plan library},
a collection of plans agents can select for execution at run time.
%
Agents' deliberation process is then aimed at selecting the most adequate plan for any event that may occur,
% so that plans are, ultimately, a way to encode specific actions to perform in response to specific events.
failing if none has been provided.

% More precisely,
\agentspeak{} plans are referred to as \emph{plan rules}, denoted as:
%
\inlineAsl{
E : C <- A$_1$; \ldots; A$_n$.
}
%
The rule describes how the agent should react to some \emph{triggering event} \inlineAsl{E},
stating for instance what to do when a new percept arises,
or how to achieve a given goal.
%
\inlineAsl{C} is the set of conditions under which the rule is applicable
(a.k.a. the rule's \emph{context}),
and \inlineAsl{A$_1$; \ldots; A$_n$} is the \emph{body} of the rule,
corresponding to the actual plan,
i.e., the actions to perform to handle the event.

Plan rules differ significantly from first-principles planning,
...
%
% Additionally,
% other than encoding the procedural knowledge to achieve goals (\inlineAsl{!Goal}),
% \agentspeak{} plan rules can be written to react to the addition or removal of beliefs (\inlineAsl{+/-Bel}),
% as well as performing \emph{epistemic actions}---e.g., a \emph{test} to retrieve information from the current belief base (\inlineAsl{?Test}).
%
% \begin{inlinelist}
...
%     due to reasoning, perception, or communication;
%     or
...
% while beliefs are logic (most commonly, Prolog) facts,
% -- i.e. the agents' belief base is considered as a logic theory --
% such as \inlineAsl{temperature(25, celsius)},
% goals are logic terms named after the activities they represent,
% such as \inlineAsl{!go(home)} or \inlineAsl{?weather(raining)}.
% %
% Plans events may be non-ground,
% in which case the plan would react to any event matching its head
% via logic unification%
% \missingref{}%
% ---e.g., the plan
% \inlineAsl{+temperature(X, celsius) : tolerance(T) \& (X > T) <- !go(away)}
% would match any update concerning the temperature.
% %
% As shown in the last example,
% plans' contexts are just logic expressions possibly involving
% the variables in the plan's head,
% or any other belief from the agent's belief base.
%
...
%     such as \inlineAsl{move(up)};
%     or
%     \item special actions for the update of believes (e.g. \inlineAsl{+Bel} or \inlineAsl{-Bel}).
% \end{inlinelist}
% %
% Actions can, and most commonly will, provoke side effects when executed,
% i.e. change either the agent's internal status or affect the environment.

Similarly to goals,
actions are basically \emph{mnemonic names} of functionalities causing effects.
%
Unlike other \ac{AI} sub-fields,
though,
...
Lastly,
a notable feature of \agentspeak{} plan rules is that the \emph{body} of a plan may contain subgoals%
---i.e., statements triggering the selection and execution of other plans.
%
Through this mechanism,
\agentspeak{} agents can pursue complex goals by breaking them down into smaller subgoals
for which the specific plans are selected at runtime,
resulting in a non-linear execution.
%
Yet,
the underlying assumption is that all plans are known in advance,
...
% \gcsidenote{'Sto capoverso è intuile, lo lascio per completezza ma è la prima cosa che salta se serve spazio}
% When encountering a subgoal during plan execution,
% a new internal event is generated inside the agent,
% which may trigger the execution of another plan,
% and so on recursively.
% %
% Stacks of goals, subgoals, and partially executed plans are called \emph{intentions}.
% %
% In a given moment,
% each agent may have multiple intention,
% each one keeping track of some course of action the agent is currently carrying on.

% In this framework,
% goals are just \emph{mnemonic names} of events the agent may be willing to react to,
% and,
% similarly,
% actions are just \emph{mnemonic names} for functionalities which provoke side effects.
%
% However
% -- differently from other sub-fields of \ac{AI} --,
% the actual semantics of goals and actions
% (namely,
% what properties of the world should be reached for a goal to hold,
% or what pre-/post-conditions is an action subject to)
% is not explicitly represented in \agentspeak{}.
% %
% So,
% it is up to human developers to write \emph{meaningful} plans, goals, and actions,
% similarly to what programmers to with ordinary programming languages.
%

% By supporting multiple concurrent intentions,
% \ac{BDI} agents can manage complex scenarios
% where multiple goals must be pursued in parallel,
% and
% where multiple courses of actions must be interleaved to pursue them all.
% %
% Yet,
% the underlying assumption is that all plans are known in advance,
% and the agent can select them based on the current context.
% %
% It may happen that a plan is not available for a given event and context,
% in which case the agent may fail to act,
% and the whole intention may remain unfulfilled.
...
%
For this paper, we highlight the main implications of integrating plan generation techniques into \ac{BDI} agents.
% which serve as a foundation for our approach as well.

In the previous section we have already surfaced the main differences between \ac{BDI} plans
and classical (a.k.a. first-principles) planning
(e.g., STRIPS~\cite{fikes1971ai})
which can be essentially be summarised as:
%
\begin{inlinelist}
    \item \ac{BDI} plans and goals are procedural, while classical planning are declarative~\cite{winikoff2002kr};
    \item \ac{BDI} actions have no clearly stated pre- and post\-conditions, while classical planning actions are defined by them
    \item \label{item:diff-subgoals} \ac{BDI} plans can include subgoals for a more flexible execution flow,
    while classical planning is usually assembling a linear sequence of actions to move from one world state to another.
\end{inlinelist}
%
\ac{HTN} planning~\cite{georgievski2015ai},
based on the idea of task decomposition,
allows generating more abstract plans that can mimic the invocation of subgoals addressing \ref{item:diff-subgoals}.
%
Nevertheless,
...
% \gcnote{
%     TODO: briefly mention \ac{AI} planning and \ac{RL}, then bring the focus to \ac{BDI} agents,
%     stressing the main analogies and (above all) differences between these approaches and \ac{BDI} plan.
%     %
%     \href{https://virtuale.unibo.it/mod/resource/view.php?id=1517191}{Andrea Omicini's slides on AI Planning in the ISE course} may be useful.
%     %
%     Then mention prior works on (automatic) plan generation in \ac{BDI} agents.
%     %
%     Authors to be considered (non-exaustive):
%     %
%     \emph{(i)} Felipe Meneguzzi, in particular papers such as \cite{MeneguzziZM04,MeneguzziZML07,MeneguzziS15,IchidaM23,XuLPM24,PereiraM24,IchidaM024};
%     \emph{(ii)} my fluffy work \cite{imagination-extraamas2021} on \ac{BDI} agents with imagination;
% }

\subsection{\ac{GenAI} Agents}

%\gcnote{
%    TODO: Overview on other approaches mixing agents (in any possible interpretation of the term) with \ac{GenAI}.
%    The point that should emerge is that \ac{BDI} and/or \agentspeak{} agents are uncovered.
%    Do not forget to mention reference \cite{IchidaM024}, among the others.
    %
%    Note: this is not merely "raw" code generation; rather, we aim to leverage the LLM's "common sense" capabilities within a planning process that can subsequently be used to write the plan.
%}
...
% \gcnote{
%     TODO: Three levels of discussion, order of presentation to be decided.
%     1. Need for a mechanism to generate plans for specific goals, via \ac{LLM} and prompt engineering.
%     2. Need to decide \emph{when} the aforementioned mechanism should be triggered.
%     3. Need to decide how plans are automatically evaluated by the agent (and possibly revised).
% }

% % \gcnote{
% %     TODO: Discuss the mechanism for item 1.
% %     The mechanism should generate one or more plans for a given goal.
% %     It should be aware of which actions and sub-plans are available to the agent and possibly reuse them,
% %     but also be open to the invention of new sub-plans.
% %     Implementation details related to \ac{LLM} exploitation are relevant here.
% % }
% % \gcnote{
% %     Can we take some inspiration from \cite{llmoracles-kbs310}?
% % }

% Long before \ac{GenAI} entered the mainstream,
% the vision of cognitive agents equipped with \emph{imagination}
% -- i.e. capable of \emph{creative} adaptation and greater autonomy --
% was already envisioned.
% %
% Previous research~\cite{llmoracles-kbs310}
% has proposed mechanisms enabling \ac{BDI} agents to engage in imaginative processes,
% thereby enhancing their cognitive and operational flexibility.
% %
% Now,
% with the proliferation of \ac{LLM} and \ac{GenAI} technologies,
% the compelling research question arises:
% can these new generative tools effectively augment agent autonomy through the dynamic generation of information?

% Attempting to answer this question,
% we investigate the integration of \ac{GenAI} into the well-established \agentspeak{}-based architecture.
% %
% Specifically,
% we seek to ensure that generative capabilities complement
% (rather than replacing)
% the fundamental \agentspeak{} structure.
% %
% The agents should thus maintain compliance with established \ac{BDI} principles,
% resorting to \ac{GenAI} exclusively when generative action is necessary.

% The next question is:
% what should \ac{BDI} agents exploit \ac{GenAI} for?
% %
% While generative and \ac{NLP} capabilities have significant potential for various aspects of cognitive agent engineering
% -- including perception, interpretation of percepts, natural-language representation of plans, and deliberation --
% our current research scope is explicitly focused on the autonomous generation of \emph{plans}.
% %
% We deliberately leave the exploitation of \ac{GenAI} for perception, deliberation, and representation to future work,
% assuming these functions remain implemented as in conventional \agentspeak{} agents.
% %
% That said,
% in the remainder of this paper,
% we refer to \emph{generative} \ac{BDI} agents as those capable of generating plans autonomously.
...
% \gcnote{
%     TODO: Enumerate the possible reasonable solutions for item 2, giving them fancy mnemonic names.
%     I can think of two major approaches:
%     a. \emph{Reactive} generation: plans are generated on-the-fly, when needed, i.e. when a plan for a goal is not available.
%     b. \emph{Proactive} generation: plans are generated by an ad-hoc plan-generation intention which is present in each generative agent since the very beginning.
%     The latter has actually already been envisioned in \cite{imagination-extraamas2021}.
%     For each solution, let's discuss the implications on the agent's design and engineering.
%     In particular, for each solution, we should discuss how the solution may "scale"
%     (creating a plan may imply creating a sub plan later on, and so on recursively).
%     c. \emph{On-demand} generation: plans are generated via some action that the agent (i.e. the programmer) can decide when to trigger.
% }
% \gcnote{
%     REMARK: Most likely, we will implement only one solution (a), so we must clarify this in the paper.
% }
...
We implement the reactive \ac{PGP} approach as described in \Cref{sec:approach} in a \jakta{} agent~\cite{DBLP:journals/sncs/BaiardiBCP24},
...
%
The setup mirrors our running example throughout the paper.
%
...
% \gcnote{
% Let's select a list of models to test.
% For each model, submit N prompts corresponding to PGP invocations.
% For each result, compute the following metrics.
% Finally, discuss results.
% - amount of generated plans (per PGP invocation)
% - average amount of beliefs per plan context
% - average amount of operations per plan body
% - amount of generated plans which are general/specific (roughly: using variables or not)
% - amount of invented goals
% - amount of invented beliefs
% - amount of generated plans which are useless (e.g., not executable at runtime, subsumed my more general plans, etc.)
% - amount of generated plans groups (PGP invocations) which “work”
%     * i.e., that EVENTUALLY lead to goal satisfaction if executed
% - distance of generated plans group w.r.t. minimal, human-derived plans group
%     * distance TBD, could be as simple as difference in amount of plans, AST comparison, etc
% - amount of inadequate usage of admissible goals
% - amount of inadequate usage of admissible beliefs
% - time to goal achievement for the generated plan groups who work (maybe normalized w.r.t. the minimal human-generated plan)
% }
