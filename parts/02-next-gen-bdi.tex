...
and a set of practical enginnering recommendations for the development of \ac{BDI} technologies, suggesting to make concurrency choices visible and controllable.
...
%
Then
we discuss the lower-level concurrency abstractions required to understand the remainder of the paper.
...
\subsubsection{External Concurrency,} conversely, is what we focus on in this paper---i.e., the way the control loops of multiple agents are mapped onto the underlying concurrency abstractions
...
In this section we identify the most relevant \emph{external} concurrency models for \acp{MAS}---namely, how the atomic parts of the agent' control loop get mapped onto the underlying abstractions
described in \Cref{sec:concurrency_abstractions}.
%
...
%
Consequently, 
we focus on the mapping of the largest possible autonomous unit in \ac{AOP}, the entire agent,
discussing the potential external concurrency models for a \ac{MAS}.
...
depending on whether the executor is fixed- or variable-size.
%
In the former case, 
...
in this paper we focus on the cases listed in \Cref{sec:taxonomy},
...
]{listings/test_output_jason_local.txt}
%
...
a template of the email sent to all maintainers can be found in \Cref{appendix:questions}.
\sidenote{La metto la mail? io sarei per toglierla completamente}
%
...
]{listings/BasicSchedulerStrategy.java}
%
