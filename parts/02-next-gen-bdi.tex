...
Modern software systems are increasingly required to manage complexity in dynamic and uncertain environments,
including physical settings in which actions can have significant consequences for the surrounding world.
A wide range of software engineering methodologies have been proposed to address this challenge,
from agent-oriented approaches to autonomic computing.
In particular, the \ac{AOP} paradigm, introduced more than thirty years ago~\cite{DBLP:journals/ai/Shoham93},
seeks to model autonomous entities with an explicit mental state,
while autonomic computing~\cite{DBLP:journals/computer/KephartC03} aims to design systems capable of self-management,
following the principles originally inspired by IBM's vision of self-* behaviour.

At the beginning of the millennium,
scenarios typical of autonomic and self-* systems appeared to be better addressed with techniques designed for multi-agent systems.
Among these, the \ac{BDI}~\cite{DBLP:conf/kr/RaoG91} model was considered particularly promising.
Yet, over time, interest in the \ac{BDI} approach within the self-* community decreased.
This trend, we argue, is not a sign that the \ac{BDI} paradigm is inherently inadequate,
but rather that the tools and frameworks available for its adoption have often been insufficient in practice.

Alongside the \ac{AOP} paradigm, autonomic computing was also inspired by a human metaphor,
this time centred on the nervous system,
which coordinates bodily functions without requiring conscious intervention.
In this setting, systems are typically characterised by self-configuration,
self-optimisation, self-healing, and self-protection~\cite{DBLP:journals/csur/HuebscherM08}.
The \mapek{} loop (Monitor, Analyse, Plan, Execute, Knowledge) captures this architecture,
and it is closely aligned with agent-based reasoning.

Despite this conceptual alignment, the literature also highlights a persistent tension between \ac{AOP} and autonomic computing.
As noted by Huebscher and McCann~\cite{DBLP:journals/csur/HuebscherM08}, \ac{AOP} is not always the most suitable paradigm for building autonomic systems,
partly because many \acp{MAS} are application-specific and lack the self-management functions that are central to autonomic computing.
This limitation is discussed also in other works that extend the classical notion of \ac{BDI} \acp{MAS} in autonomic settings~\cite{DBLP:conf/prima/LiaoJG06},
proposing PDC-agents (policies, desires, and contracts) to address the effects of environmental change on agent behaviour~\cite{DBLP:conf/smc/JunJZBCJ04}.

In this part, we argue that the reason \ac{BDI} has not been more central to the development of autonomic computing systems is not primarily the abstractions themselves,
but rather the lack of practical tools that support their integration with real systems.
The challenge is not only conceptual: it also concerns interoperability between autonomic control components,
for which \ac{BDI} may be well suited,
and the systems they manage,
which are often expressed through more conventional abstractions and frameworks.
Additionally,
learning \ac{BDI} requires considerable effort,
as it introduces new paradigmatic elements unknown to most computer scientists,
and, typically, does so through a custom syntax;
in other words, the learning curve is \emph{steep}.
%
On top of this, developers willing to learn a new paradigm and related syntax
may still end up being discouraged from using the paradigm by the lack of industrial-grade
tooling (\acp{IDE}, debuggers, static analysers, build systems, testing frameworks, and so on).
...
%
In \Cref{chap:concurrency},
we discuss the importance of carefully managing concurrency in \acp{MAS},
distinguishing between \emph{internal} and \emph{external} concurrency.
%
In doing so, we analyse the state of the art of \ac{BDI} frameworks and highlight how they address concurrency.
%
Building on this paradigm-blending approach, 
\jakta{} enables us to explore the integration of \ac{GenAI} into \ac{BDI} agents,
as discussed in \Cref{chap:genai},
to investigate how \acp{LLM} can support plan generation in unknown environments.
%
In the final chapter of this part, \Cref{chap:perspectives},
we summarise the research outcomes and outline the direction for building the next generation of \ac{BDI} agents,
while also offering a perspective on future research directions for further improving the \ac{BDI} paradigm and its adoption within the self-* systems community.
...
The remainder of the chapter describes the \ac{DSL} design and the interpreter architecture (Section~\ref{sec:jaktadsl}), illustrates the approach via examples (Section~\ref{sec:running-example}), and discusses limitations and future directions (Section~\ref{sec:jakta:conclusion}).
...
\section{Conclusion, limitations, and future work}\label{sec:jakta:conclusion}
...
We start this chapter describing the terminology we rely on (Section~\ref{sec:concurrency:background}) and then present the taxonomy of external concurrency models (Section~\ref{sec:taxonomy}). Section~\ref{sec:comparison} applies the taxonomy to a set of real frameworks and discusses practical trade-offs; Section~\ref{sec:discussion} draws lessons and recommendations for framework design and use.
%
...
\section{Background}\label{sec:concurrency:background}
...
and in \Cref{sec:genai:conclusion} we conclude and outline future work.

\section{Background}\label{sec:genai:background}
...
\section{Conclusion}\label{sec:genai:conclusion}
...
and the main principles of \ac{hMAS} in \Cref{sec:hypermas:background}.
%
...
This part of the thesis has proposed some opportunities 
for advancing \ac{BDI} agents in modern software environments.
%
First,
it has shown with the \jakta{} prototype 
that \ac{BDI} agents can be integrated 
with mainstream programming languages 
to provide a more expressive and accessible programming environment 
while not loosing the core abstractions of \ac{BDI} systems.
%
This improves modularity, reuse, and maintainability, 
while lowering the entry barrier.
%
Second,
it has investigated how concurrency is managed in \agentspeak{} frameworks,
proposing a dichotomy between \emph{internal} and \emph{external} concurrency.
%
Then,
it has investigated how \ac{GenAI} can be integrated with \ac{BDI} agents to support plan generation at runtime,
showing a \jakta{} prototype implementation of a plan generation pipeline that leverages \acp{LLM} to generate candidate plans when no suitable plan is available in the library.
%
The analysis addresses the main questions surrounding this integration:
what information is required to generate admissible plans,
how knowledge should be transferred between the model and the agent,
what changes are introduced to the agent specification,
and whether the resulting plans can be reusable.
%
Finally,
it has investigated how the paradigm could be integrated with semantic hypermedia and the Web,
proposing a generic \ac{BDI} engine that separates the reasoning cycle from the concrete representation of beliefs and actions,
thus allowing reasoning on any knowledge representation, other than \ac{OWL} and \ac{RDF}.

Taken together, these contributions point to a common conclusion:
\ac{BDI} agents remain a powerful and principled foundation for intelligent systems,
but their practical adoption depends on their ability to evolve with software engineering practices.
%
The next generation of \ac{BDI} systems should be designed as open, adaptable, and interoperable cognitive architectures,
capable of integrating multiple paradigms rather than relying on a single, rigid execution model.

%% Future work: make a heading


\section{Cognitive Agents in the Field: a Hybrid Approach Between Agent-Oriented and Aggregate Computation}

Engineering self-* systems
necessitates robust abstractions that can bridge the gap between 
conceptual understanding 
and practical implementation.
%
Software modeling has emerged since the very beginning 
of the \ac{SASO} systems community 
as a fundamental pillar for understanding, 
designing, 
and implementing complex systems~\cite{karsai1999saso}. 
%
Nowadays,
software modeling has become an essential component of the entire software development lifecycle,
facilitating both human understanding and computational processing.
%

Models play a crucial role in computer science research for several interconnected reasons:
%
\begin{inlinelist}
\item they bridge the abstraction gap between 
high-level conceptual understanding 
and low-level implementation details, 
enabling researchers to focus on essential system properties;
\item they enable reproducibility in research by providing precise specifications that can be independently validated and compared across different studies;
\item they serve as development guides that inform architectural decisions and implementation strategies, reducing the likelihood of design errors and facilitating systematic software construction;
\item they provide the flexibility needed to adapt to changing requirements, allowing systems to evolve while maintaining their essential properties and behaviors.
\end{inlinelist}

In the context of \ac{SASO} systems, 
two distinct yet complementary perspectives have emerged 
that address different aspects of system complexity.
%
The \emph{singular perspective}, 
focuses on modeling individual entities (i.e. devices, agents),
providing rich abstractions for modeling their behavior.
%
Different modeling approaches have been proposed in this perspective, 
coming from different research communities.
%
Some examples of such approaches include \mapek{}~\cite{DBLP:journals/computer/KephartC03}, and \ac{PRS}~\cite{DBLP:conf/aaai/GeorgeffL87}.
%
Conversely, the \emph{aggregated perspective},
emphasizes collective behaviors and emergent properties 
that arise from the interaction of several entities.
%
In this perspective,
the focus is on modeling the collective phenomena,
simplifying the singular agents abstractions.
%
An example of the latter is the \ac{AC} paradigm~\cite{BealIEEEComputer2015}.

While both perspectives offer valuable modeling capabilities, 
they are typically applied separately in practice, 
often favoring either an advanced individual approach or a collective one.
%
We recognize that individual agents and collective behaviors 
are not competing abstractions but complementary aspects for the design of complex adaptive systems.
%

This section bridges together cognitive \ac{BDI} \ac{AOP} and \ac{AC}, 
creating a unified framework that leverages the strengths of the both.
%
The integration of \ac{AC} and \ac{AOP} was first explored in~\cite{viroli2017ijaose}, 
which examined the incorporation of aggregate plans within \ac{AOP} \ac{MAS}.
%
Building on the future directions outlined in that work, 
we advance the integration by adopting the \ac{BDI} framework as the abstraction for \ac{AOP}, 
and we outline a roadmap for its realization.
%
Specifically, 
we leverage existing frameworks for both \ac{BDI} and \ac{AC} abstractions.

% ============================
% CONTRIBUTION AND OBJECTIVES
% ============================
\subsection{Contribution and Objectives}\label{sec:contribution}
% present the main contribution of the research in a way that a non-expert could clearly understand, then present the detailed objectives of your research, highlighting why your work is novel in comparison to existing research.

This section proposes to investigate a novel cognitive agent-oriented framework that seamlessly integrates 
individual decision-making capabilities with distributed collective computation. 
%
The aim is to provide a hybrid framework that combines the \ac{BDI} \ac{AOP} for local cognitive reasoning 
with \ac{AC} mechanisms for collective coordination.
%

\subsubsection{Hybrid Cognitive-Collective Framework}
Unlike tools that treat individual and collective intelligence as separate paradigms, 
our proposal provides a unified framework that seamlessly integrates both perspectives.
%
% This integration enables to adopt sophisticated modelling abstractions for both
% individual cognitive reasoning and collective computation.
%
% Depending on the goal requirements and environmental conditions,
% developers can preserve a cognitive model abstraction for defining individual agents' behaviour
% while allowing them to benefit from distributed collective capabilities.

The proposed framework is built upon three fundamental features:

\paragraph{Cognitive Local Decision-Making}
Individual agents employ the \acf{BDI} \ac{AOP} cognitive framework to devise locally-relevant decisions.
%
Inspired from \ac{PRS}, 
\ac{BDI} has its roots in philosophy, 
where Michael Bratman described humans’ reasoning via the
“beliefs, desires, intentions” framework, as a way to explain future-directed
decision-making~\cite{bratman1987intention}. 
%
Successively, the framework
turned into an abstract semantics for computational agents: \agentspeak{}~\cite{DBLP:conf/maamaw/Rao96}.
%
With this model, 
each agent maintains its own beliefs about the environment (i.e. its local perception), 
goals, and intentions that guide the sequence of actions.
%
% The main advantage of this cognitive architecture is a fine-grained plan engineering which relies on agents' local perception.
%

\paragraph{Collective-Oriented Task Delegation}
When dealing with tasks that exceed the capabilities of individual perspective
or benefit from collective intelligence, 
modelling singular agents falls short.
%
% This delegation allows the system to leverage the collective computational power and distributed sensing capabilities of multiple devices in the environment.
%
%
Tasks such as crowd management~\cite{BealIEEEComputer2015}, 
application-level network protocols~\cite{DBLP:conf/acsos/BaiardiPAT25}, 
or swarm-drone coordination~\cite{DBLP:conf/acsos/GrushchakKPFABS24}, 
are some examples of successful application of \ac{AC}.
% where the emergent behavior of the collective exceeds what individual agents could achieve independently.
%
\ac{AC}~\cite{BealIEEEComputer2015} is a functional macro-programming paradigm
centered around the notion of a \emph{computational field}~\cite{DBLP:journals/tocl/AudritoVDPB19}:
a distributed data structure that maps points in space and time to values.
%
% Building on computational fields, \ac{FC}~\cite{DBLP:journals/tocl/AudritoVDPB19}
% extends this idea through higher-order functional programming abstractions,
% allowing manipulation and composition of fields
% based on local observations of their values.

\paragraph{Bidirectional Information Flow}
The proposed framework establishes a bidirectional information flow 
between local cognitive agents and the aggregated computing layer.
%
On the one hand, agents gather information from swarm computation 
and exploit collective intelligence to enhance their individual reasoning processes.
%
% This integration is driven by each agent's own perception of the environment, 
% ensuring that collective insights are appropriately contextualized within the agent's local decision-making framework.
%
On the other hand, 
the collective layer is informed by the individual situated perceptions of participating agents.

This framework improves the engineering of \ac{SASO} systems, 
by providing a unified framework that leverages the strengths of both paradigms,
thus enabling the development of more sophisticated and adaptive systems that can operate effectively across a wide range of scenarios and scale requirements.




\subsection{Methodology}\label{sec:methodology}
% present the methodology of your approach, discussing why it is suitable to your contribution, and also present any preliminary results if you have them, including a statement about the current status of your work.

To address the features described in Section~\ref{sec:contribution}, 
we propose to leverage the modeling capabilities of two frameworks: 
\ac{jakta} for \ac{BDI} \ac{AOP} and Collektive for \ac{AC}.
%

\ac{jakta}\footnote{https://github.com/jakta-bdi/jakta} integrates \agentspeak{} semantic features with Kotlin-native mechanisms, 
meant to improve \ac{BDI} ergonomics with pluggable concurrency and communication management constructs~\cite{DBLP:journals/sncs/BaiardiBCP24}.

Collektive\footnote{https://github.com/Collektive/collektive} is a prototype framework for \ac{AC} based on \ac{XC}~\cite{audrito2024jss},
which extends the Kotlin compiler plugin for performing \ac{AC} \emph{alignment}~\cite{audrito2016saso}.
%
The alignment is the mechanism that allows communication between devices (or, agents),
without an explicit notion of message communication, 
by exploiting \ac{AST} annotations.
%

Both technologies are built on top of Kotlin and distributed as open-source libraries,
and provide an internal \ac{DSL} that allows developers to write programs in a more expressive way.
%
The \ac{DSL} design enhances the ergonomics of programming in the respective framework,
while still allowing the use of Kotlin's powerful type system and paradigm blending features (i.e. the seamless usage of \ac{OOP} and \ac{FP} constructs).
%
The choice of Kotlin as the underlying language is strategic,
since it allows reuse of existing tools and libraries (i.e. \acp{IDE}, syntax highlighters, ...),
which are essential to support the development process of complex software systems.

Since both \ac{jakta} and Collektive are built on top of Kotlin,
we will leverage Kotlin's type system and \ac{DSL} capabilities to create a unified
framework that integrates cognitive and aggregate paradigms.
%
The expected result is a unified framework for modelling cognitive agents
that can invoke collective computation as a natural extension of \ac{BDI} action capabilities.
%
% At the cognitive level, 
% agents reason over their events and perceptions using 
% \ac{jakta} \ac{BDI} abstractions.
% %
% At the collective level, 
% aggregate computations can be defined using directly Collektive model abstractions,
% %
% At the hybrid level, 
% agents can invoke Collektive computations to perform tasks that require collective intelligence,
% gathering the outcomes of the computation to enhance their local decision-making.

% Information flow will be modeled using Kotlin's type system to create compile-time verified specifications.

% ============================
% FUTURE WORK AND RESEARCH PLAN
% ============================
\subsection{Future work and Research plan}
% summarise your intended future work and provide a schedule of milestones with a discussion of their feasibility.

The main challenge of this integration lies in 
the logic-based nature of \ac{BDI} reasoning,
which is inherited from the \agentspeak{} semantics.
%
Since proposed as an implementation of \agentspeak{},
current \ac{jakta} implementation comes with its own logic reasoning engine.
%
Collektive,
on the other hand,
operates in terms of \emph{computational fields}, 
which are distributed data structures
that conceptually map every device to a value produced in
a program. 
%
Therefore,
the first step for the definition of this framework is to separate the tight coupling between logic constructs and the \ac{jakta} implementation.
%
This separation will allow \ac{jakta} to be used as a general-purpose programming language for \ac{BDI} agents,
extending its reasoning process to reason over any kind of structured information other than the logic one.

Once this is achieved,
\ac{jakta} and Collektive can be easily integrated,
since the first can invoke aggregate plans and then reason on its outcome.

Finally,
to validate the hybrid framework,
we will integrate it with the Alchemist Simulator\footnote{https://github.com/AlchemistSimulator/Alchemist},
which is a \ac{DES} engine for simulating large-scale distributed systems~\cite{PianiniJOS2013}.
%
The advantage of using Alchemist is twofold:
\begin{inlinelist}
    \item It will enable simulation of non-trivial large-scale distributed systems, 
    acting as a \emph{debugging} tool for this framework, and
    \item It will enable direct simulation of \ac{DSL} definition of the system, 
    thus validating the same codebase that will be executed in real-world deployment.
\end{inlinelist}
%
Simulation will enable the evaluation of 
scalability, 
robustness, 
efficiency, 
and convergence time 
in systems modeled with the proposed framework, 
thereby assessing not just application performance 
but the framework’s own expressiveness and its advantages relative to existing approaches.

\section{Learning enabled hybrid multi-agent coordination for resilient self-integrating systems}

Modern cyber-physical 
and 
socio-technical systems~\cite{ZENG20201028} are growing in 
scale, openness, and dynamism 
faster than their behaviour can be specified at design time~\cite{Sztipanovits03072019}. 
Increasingly, 
such systems take the form of 
large populations of autonomous agents 
that 
share an environment but differ in 
their capabilities, 
communication norms, 
and 
objectives. 
%
Transportation is a representative example: 
maritime vessels, ground vehicles, and aircraft 
increasingly operate as fleets of autonomous agents, 
yet 
their coordination remains 
domain-specific, 
architecturally centralised, 
and 
unable to transfer experience across operating contexts.

%% __ CHALLENGES ________________________________
For systems of this kind, 
anticipating every operating condition at design time 
is no longer feasible, 
and 
two limitations of current practice stand out. 
%
First, 
centralised coordination creates a performance bottleneck 
and 
a single point of failure, 
and 
does not scale to fleet-level deployment. 
%
Second, 
agents designed or trained in isolation degrade when 
they meet conditions outside their design assumptions%
---new actors, infrastructure changes, or cross-domain situations. 
%
What is required are therefore
%not merely better individual agents,
%but
systems that continuously 
self-adapt and self-integrate at runtime as agents 
join, leave, are upgraded, or interact across domain boundaries.

%% __ RELEVANCE TO SISSY ________________________________
This is precisely the concern of the \ac{SISSY} initiative, 
which seeks to move integration-related decisions 
from 
design time to runtime, 
shifting expertise and responsibility 
from 
human engineers to the system itself~\cite{bellman2021self}. 
%
It continues a longer tradition of 
self-adaptive and self-organising systems%
--autonomic and organic computing--%
that 
manage complexity through runtime self-management 
rather than 
fixed design~\cite{DBLP:journals/computer/KephartC03,muller2017organic}.

%% __ SUGGESTED SOLUTION: PARADIGM FUSION ________________________________
We observe a structural gap at 
the intersection of three research traditions that, 
combined, could meet this need:
\begin{itemize}
    \item \textbf{\acf{AC}}~\cite{BealIEEEComputer2015,viroli2019jlap} 
    programs collective behaviour at the aggregate level, 
    resolving heterogeneity at the coordination-language level 
    without requiring homogeneous agent implementations.

    \item \textbf{\acf{AMAS}}~\cite{gleizes2007engineering} 
    focus on local agent behaviour to ensure 
    adaptation, robustness, and self-repair 
    through cooperative behavioural rules 
    and environment-driven self-organisation.

    \item \textbf{\acf{FMARL}}~\cite{jing2025federated} 
    lets agents share learned models across organisational 
    and domain boundaries without centralising raw experience data.
\end{itemize}

%% __ CONTRIBUTION/NOVELTY ________________________________
These paradigms are typically studied in isolation, 
and 
their integration into a single self-adaptive architecture 
remains largely unexplored. 
%
In this section, we propose such an integration, 
grounded in self-adaptive systems and targeting precisely this gap.
%
Our contributions are:
\begin{enumerate}[label=\alph*)]
    \item a three-layer reference architecture for the self-improving integration 
    of heterogeneous \ac{MAS}, 
    combining 
    collective coordination, cooperative self-organisation, and federated learning, 
    with 
    an explicit account of how each layer compensates the limitations of the others;

    \item the identification of \emph{situation characterisation}
    -- deciding at runtime which paradigm should govern a given situation --
    as the central open challenge 
    that 
    makes the integration work, together with the further challenges it raises; 
    and

    \item an illustrative transportation scenario (highway platooning) 
    that 
    shows the architecture in operation.
\end{enumerate}

%% __ STRUCTURE ________________________________
The remainder of this section is organised as follows. 
\Cref{subsec:background} 
reviews the state of the art across self-adaptation 
and 
the three contributing paradigms. 
%
\Cref{subsec:architecture} 
presents the three-layer architecture. 
%
\Cref{subsec:challenges} 
sets out the central research challenge and those derived from it. 
%
\Cref{subsec:domain} 
illustrates the architecture on a platooning scenario, 
and 
\Cref{subsec:conclusion} 
...
%% NOTE: Revised by Ghassan

Self-adaptation is widely recognised as a key approach 
to address 
the growing complexity of modern applications. 
%
A self-adaptive system is considered to lie somewhere 
between two categories of systems \cite{delemos2013swese}. 

%% top-down: Motivation >>
On one hand,
there are self-adaptive systems driven by a central controller,
which evaluates and monitors the system and the operating context
to decide whether to trigger adaptations.
%% top-down: Centralised >>
These systems 
typically 
rely on an explicit internal representation 
of themselves and their environment. 
%
Although 
this approach provides 
strong control over the adaptation process 
and 
facilitates verification and predictability, 
it also presents several limitations 
when dealing with large-scale 
and 
highly dynamic applications. 
%
In particular, 
the central controller may become a performance bottleneck 
and 
a single point of failure~\cite{lange2025measurement}.
%
Maintaining an accurate global model of 
the system and its environment 
can be computationally expensive 
and 
increasingly difficult as the system grows in size and complexity.
%
In addition, 
centralised adaptation mechanisms 
often struggle to cope with 
uncertainty, incomplete information, and rapidly changing environments, 
thereby 
limiting their scalability, flexibility, and robustness (see~\cite{al2026collaborative}).

%% bottom-up: Motivation >>
On the other hand, 
there are self-adaptive systems designed to function in a decentralised fashion.
%% bottom-up: Pro >>
These systems operate without a central authority 
and 
are typically made up of a large number of components 
interacting locally according to simple rules. 
%
The global behaviour of the system emerges from these local interactions.
%
Examples of these systems include nature-inspired and organic computing systems,
with coordination mechanisms taking inspiration from physics~\cite{DBLP:journals/tosem/MameiZ09,viroli2019jlap},
chemistry~\cite{DBLP:conf/coordination/ViroliC09},
biology~\cite{schranz2021swevo},
and ecology~\cite{zambonelli2015percom,DBLP:journals/nc/BriscoeSW11}.
%
By design,
these systems generally exhibit high
scalability, robustness, and flexibility.

Provided the importance the local interactions have in this kind of system,
their design is often driven by the creation of local rules,
in a bottom-up fashion.
%
However,
systems designed in this way may be difficult to modularise and reuse~\cite{DBLP:conf/icse/BozhinoskiB18}.
%
In general,
there are few guarantees that the combination of multiple local rules
coming from different sources will lead to a desired combined global behaviour~\cite{DBLP:journals/swarm/ReinaMDT15}.
%
It is generally difficult to predict the global behaviour of a system in the absence of a global view,
which in turn may lead to suboptimal decisions,
or conflicts between local objectives~\cite{DBLP:journals/firai/Kuckling23,DBLP:journals/firai/RoliLB19}.

\acp{MAS}~\cite{wooldridge2009introduction} are a large family of approaches that
has been widely investigated as self-* systems 
able to self-adapt the system during runtime 
lying between the afore-mentioned categories. 
% Our Goal in contract >>
In this work, 
we investigate the integration of three families of \ac{MAS}-based approaches: \ac{AC}, 
\ac{AMAS}, 
and 
\ac{MARL} together with its federated extension.

\subsubsection{\acl{AC}}

\ac{AC}~\cite{BealIEEEComputer2015} 
is an approach meant to design self-organising systems
from a global perspective.
%
Compared to many rule-based frameworks, 
a key element of \ac{AC} is the usage of functional languages in place of middlewares.
%
In other words, 
\ac{AC} is a programming paradigm where the logical machine being programmed shifts
from the single device to the network of devices.
%
\ac{AC} languages manipulate distributed data structures 
mapping devices to values, called fields, 
through functional operators~\cite{DBLP:journals/tocl/AudritoVDPB19}.
%
A central aspect of this approach is 
that well-behaved (typically, self-stabilising~\cite{DBLP:journals/cacm/Dijkstra74})
patterns can be modularised as functions and reused,
building libraries of reusable building blocks~\cite{DBLP:journals/tomacs/ViroliABDP18}.

In \ac{AC},
all agents execute the same programme indefinitely in repeated asynchronous rounds.
%
Each round comprises the following phases:
\begin{itemize}
  \item \emph{sense}: 
  each agent perceives 
  the environment's information and the messages coming from neighbouring agents
  \item \emph{compute}:
  each agent runs the aggregate programme
  using the data collected during the \emph{sense} phase;
  \item \emph{share/act}:
  each agent sends messages to neighbouring agents with the computation result.
\end{itemize}

% There exist several implementations for \ac{AC}, 
% each one targeting a different platform.
% %
% Some examples include Protelis~\cite{pianini2015protelis} Java-based implementation,
% ScaFi~\cite{casadei2022scafi} Scala framework,
% and 
% Collektive~\cite{cortecchia2024multiplatform} Kotlin multiplatform implementation.
% %

%% AC LIMITATIONS >>
However, 
the strengths of the paradigm comes with a drawback:
devices are typically assumed to be 
homogeneous 
%and to execute the same program,
which makes it 
hard to naturally express and support heterogeneous capabilities and behaviours within \ac{AC}.
%
What is needed is an approach that 
preserves the high-level expressiveness and coordination abstractions of \ac{AC} 
while 
...
to 
participate and contribute according to their individual capabilities.

\subsubsection{Adaptive Multi-Agent Systems}
The \ac{AMAS} theory \cite{gleizes1999theory,gleizes2007engineering} 
is a bottom-up design approach 
in which cooperation drives the system's self-organisation. 
%
Starting from the autonomous entities of the 
target problem and their interactions, 
the theory guides the design of 
local cooperative agent behaviours 
from which a solution to the global problem emerges.

In \ac{AMAS},
cooperation is defined as 
the ability of agents to work together to achieve a \emph{common global objective}.
%
To this end, 
the design process relies on 
analysing agent behaviours across a variety of execution scenarios 
in order to identify situations where cooperation fails. 
%
The system designer 
must then generate agents with mechanisms that 
enable them to resolve these non-cooperative situations autonomously. 
%
Three generic adaptation mechanisms have been defined~\cite{bonnet2017multi}:
\begin{itemize}
    \item Reorganisation: 
    the agent modifies its relationships with other agents by 
    establishing new interactions, 
    terminating existing ones, 
    or adjusting the level of trust associated with its current relationships.
    
    \item Adjustment: 
    the agent adapts its behaviour by modifying its internal parameters.
    
    \item Openness: 
    the agent decides either to create a new agent or to remove itself from the system.
\end{itemize}

Through these continuous and autonomous adaptations, 
agents collectively enable the system to self-adapt to changes in its environment. 
%
In other words, 
the cooperative design of individual agent behaviours constitutes 
the driving mechanism for both 
the continuous adaptation of agents 
and 
the autonomous organisation of the overall system.

%% AMAS LIMITATIONS >>
However, 
achieving such adaptive behaviour requires 
careful calibration by the system designer. 
%
In particular, 
the designer must determine 
\begin{enumerate*}[label=(\roman*)]
    \item the local information that should be accessible 
    to each agent to support effective decision making, 
    and
    \item  sufficiently comprehensive set of execution scenarios 
    to identify and address potential cooperation failures.
\end{enumerate*} 
%
Both aspects are highly 
application-dependent 
and 
rely heavily on the designer's expertise, 
making the autonomous integration and interoperability 
of independently designed \ac{AMAS}-based systems particularly challenging.


\subsubsection{\acl{MARL}}
\ac{RL} 
allows an agent to learn a decision-making policy through 
trial-and-error interaction with its environment, 
guided only by a scalar reward signal~\cite{sutton2018reinforcement}.
%
\ac{MARL} extends this principle 
to settings in which several agents learn concurrently 
within a shared environment~\cite{busoniu2008comprehensive}. 
%
Depending on how their rewards relate, 
agents may be 
fully cooperative, 
fully competitive, 
or act in mixed settings, 
which 
makes \ac{MARL} a natural framework for modelling coordination among autonomous entities.

A difficulty specific to \ac{MARL} is non-stationarity: 
as every agent updates its own policy, 
the environment perceived by each individual agent keeps changing, 
which weakens the convergence guarantees available in the single-agent case~\cite{busoniu2008comprehensive}. 
%
A widely adopted response is the paradigm of 
centralised training with decentralised execution, 
in which agents exploit shared information 
while learning but act only on local observations at runtime~\cite{lowe2017multi}. 
%
Building on deep neural networks, 
recent \ac{MARL} methods have addressed increasingly realistic problems 
involving large agent populations 
and 
high-dimensional perception~\cite{gronauer2022multi}.

The main strength of \ac{MARL} is 
that it learns coordinated behaviour directly from experience, 
without requiring designers to anticipate and encode every situation the agents may face.
%% MARL LIMITATIONS >>
This same property is also its main weakness. 
%
Achieving robust performance generally demands 
a large and diverse set of training scenarios, 
which is costly in 
data, computation, and energy. 
%
Furthermore, 
the resulting policies are hard to interpret and to verify, 
which is 
a serious obstacle in safety-critical applications 
where 
guarantees on behaviour are required~\cite{gronauer2022multi}.



\subsubsection{\acl{FL} for \acl{MAS}}
\ac{FL} 
is a distributed machine-learning setting in which 
many clients collaboratively train a shared model 
under the coordination of a server,
while keeping their training data local~\cite{mcmahan2017communication}. 
%
Only model updates, rather than raw observations, 
are exchanged and aggregated, 
which 
reduces communication cost 
and 
limits the exposure of private or proprietary data~\cite{kairouz2021advances}.
%
These properties are attractive for \ac{MAS}, 
whose agents are often owned by different 
stakeholders, operate across organisational or domain boundaries, 
and 
cannot share their raw experience for reasons of 
privacy, bandwidth, or regulation.

%% FMARL = MARL + FL
\ac{FMARL} combines this principle with \ac{MARL}: 
each agent learns locally from its own interactions 
and 
periodically shares the parameters of its learned model, 
so that the collective benefits from 
\emph{experience gathered across the whole population} 
without centralising it~\cite{jing2025federated}. 
%
A further practical advantage is that 
a newly joining agent can \emph{inherit the shared model} 
instead of learning from scratch, 
which 
accelerates its integration into the system.

%% FMARL LIMITATION >>
The main open difficulties of \ac{FMARL} stem from heterogeneity. 
%
Agents typically observe non-identically distributed data 
and may differ in their 
sensing modalities, action spaces, and roles, 
which 
makes the aggregation of their models 
far from straightforward~\cite{kairouz2021advances,jing2025federated}. 
%
In addition, 
the quality of the shared model still depends on 
the diversity of the situations encountered across the federation, 
and 
the aggregation step must remain robust to unreliable or poorly performing contributors.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                       Framework description                                 %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The three-layers architecture vision}
\label{subsec:architecture}
%% NOTE: Revised by Ghassan

The approaches discussed in \Cref{subsec:background} 
are typically investigated independently, 
despite their potential to complement one another.

Each approach captures only a subset of the complexity inherent to adaptive \acp{MAS}.
%
Indeed, 
each is designed to address a specific challenge 
while abstracting away other aspects 
that remain relevant to the overall system behaviour.
%
Specifically, 
\ac{FMARL} 
focuses on tuning system parameters based on 
previous executions within a bounded environment; 
%
\ac{AMAS} 
models the behaviour of heterogeneous autonomous entities 
operating in unknown environments; 
%
and 
\ac{AC} 
captures the emergent behaviour arising from the interactions 
of large numbers of homogeneous devices.

In this section, 
we argue that these approaches can be integrated into a unified framework 
that 
leverages the strengths of each to compensate for the limitations of the others.
%
Furthermore, 
we contend that relying on a single modelling abstraction 
is insufficient to capture the full complexity of adaptive systems, 
potentially leading to 
deployments that lack the resilience required 
for operation in dynamic and uncontrolled environments.

To address this limitation, 
we propose 
the layered architecture illustrated in \Cref{fig:architecture}, 
in which the aforementioned approaches interact with one another 
in addition to acting directly on the underlying devices. 
%
In the following sections, 
we describe how each layer benefits from the others 
within this unified architectural framework.

\begin{figure}
    \centering
    \includegraphics[width=0.4\linewidth]{figures/sissy26/schema-layered}
    \includegraphics[width=0.48\linewidth]{figures/sissy26/elsyFig1}
    \caption{%
        The layered framework architecture proposal (left) and the interlayer information/control exchanges (right).
        The \emph{\acl{AC}} layer provides
        collective coordination, and builds collective concepts such as teams,
        or even aggregate visions of the entire system.
        %
        The \emph{Self-Organising} layer provides heterogeneous agent-level coordination;
        crucially, individual agents may be teams of devices behaving as a single entity,
        as produced by the \ac{AC} layer.
        %
        The \emph{Federated MARL} layer continuously improves
        shared policies from operational data
        and
        intervention events.}
        % devices, teams of devices, and sys behaviours.
        % The \emph{Federated MARL} layer continuously improves
        % shared policies from operational data and
        % intervention events.}
    \label{fig:architecture}
\end{figure}

\subsubsection{\acl{AC} and self-organising layer}

\ac{AC}
offers a powerful abstraction 
for specifying collective behaviour over large populations of
homogeneous agents through
distributed, field-based computations.
%
As discussed in \Cref{subsec:background}, 
its homogeneous programming model makes it difficult to 
represent systems whose entities differ in 
capabilities, objectives, or decision processes.
%
The \ac{AMAS} layer 
addresses exactly this gap: 
it supports heterogeneous autonomous agents through 
cooperative local behaviours, 
letting each agent adapt to its own role and constraints.
%
Crucially,
\ac{AC} can be leveraged to create entities that are treated as single-agent at the \ac{AMAS} layer,
despite being composed of multiple devices.
%
For instance, a swarm of drones that must stay in formation could be programmed using \ac{AC}
(there are dedicated frameworks, such as Macroswarm~\cite{DBLP:journals/lmcs/AguzziCV25}),
and the swarm can then be treated as a single agent at the \ac{AMAS} layer (bottom-up construction).
\ac{AMAS}-level swarm commands get automatically translated into control signals for each device by \ac{AC} (top-down control).

The combination of \ac{AC} and \ac{AMAS} enables both local interactions and emergent behaviour: 
agents make decisions based solely on local information 
while collectively contributing to a shared global behaviour. 
%
This coordination is facilitated by \ac{AC}, 
which provides a shared representation of the environment through a spatio-temporal field,
where information is continuously propagated and updated so agents can indirectly coordinate their actions without requiring explicit communication or centralised control.



\subsubsection{The self-organising layer and \ac{FMARL}}
The \ac{FMARL} layer 
builds on the reinforcement-learning policies running at agent level, 
leveraging local behaviours through 
shared policies learned from operational data or generated scenarios. 
%
\ac{FMARL} 
is well suited to learning complex coordination policies 
without explicit expert knowledge, 
but 
robust performance generally requires a large and diverse set of training scenarios, 
which is 
costly in time, memory, and energy. 
%
Conversely, 
the \ac{AMAS} layer relies on expert-designed cooperative behaviours 
that 
are transparent, interpretable, and easy to validate; 
its limitation is that experts must anticipate the situations agents may meet 
and 
design behaviours for them, including rare or complex cases that are hard to foresee.


% COUPLING >
% \gaf{Coupling of FMARL and the AMAS is presented, but coupling AMAS and AC is missing!}
% \dpi{Coupling described in the previous section.}
% \gaf{The limitations of each was described, introducing to the urge/logic of coupling suggested here.}
Coupling the two paradigms relieves this tension. 
%
The expert specifies cooperative behaviours 
for 
the most frequent and well-understood situations, 
so 
the self-organising mechanisms handle the majority of operation, 
while 
\ac{FMARL} is dedicated to learning policies 
for the remaining 
uncommon, ambiguous, or highly dynamic situations, 
where handcrafted behaviours would be impractical. 
%
The learning problem is thereby restricted to a much smaller part of the state space, 
which 
sharply reduces the data and computation required 
while 
preserving the adaptability of the whole system.
...
%% NOTE: Revised by Ghassan and Elsy

Integrating the three paradigms shifts 
the central difficulty from designing or learning all agent behaviours 
to 
deciding, at runtime, 
which 
paradigm should govern a given situation. 
%
We single out this question as the main open challenge, 
and 
then outline three further challenges that the integration raises.

\subsubsection{Situation characterisation}
The proposed architecture exploits 
expert knowledge wherever it is available 
and 
delegates only uncertain situations to \ac{FMARL}.
%
\ac{RL} 
is therefore no longer expected to learn the complete system behaviour, 
but only to complement the self-organising mechanisms 
in regions of the state space where expert knowledge is incomplete. 
%
This improves learning efficiency 
while 
preserving the interpretability and robustness of the overall system.
%
% CHALLENGE OF AMAS+FMARL
The price is a new decision problem: 
combining \ac{FMARL} with \ac{AMAS}-based self-organisation 
moves the challenge from designing or learning every behaviour 
to 
determining when each paradigm should apply. 
%
The architecture thus needs a mechanism to 
characterise the situations agents encounter 
and to 
select the most appropriate decision-making paradigm%
---distinguishing situations that the expert-designed behaviours can reliably handle 
from those 
that fall outside their scope, 
where the learned policies must take over. 
%
This situation characterisation acts as a decision layer 
that dynamically selects the control strategy 
...
The integration raises three further challenges. 
% CHALLENGE 1:
\emph{Governance and minimal intervention}: 
deciding 
when and how 
a higher layer should intervene in the collective 
without re-centralising control or undermining agent autonomy. 
%
% CHALLENGE 2:
\emph{Robust federated aggregation}: 
combining models learned by agents that observe non-identically distributed data 
and 
differ in their observation and action spaces, 
while 
remaining robust to unreliable contributors~\cite{kairouz2021advances,jing2025federated}. 
%
% CHALLENGE 3:
\emph{Multi-objective evaluation}: 
assessing self-improving \ac{MAS} against 
competing objectives%
--safety, efficiency, coordination resilience, and fairness--%
whose trade-offs must be made explicit 
and 
measured consistently at both global and local levels~\cite{lange2025measurement}.
...
\label{subsec:domain}
%% NOTE: Revised by Ghassan
% \eka{Polish and Confirmation requried for the revised version}

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/sissy26/framework.pdf}
    \caption{%
        % \gaf{Other representation for self-standing figure?}
        The proposed layered architecture applied to a highway platooning scenario. Autonomous vehicles dynamically self-organise into platoons through \ac{AC}, while \ac{AMAS} mechanisms operate on both individual vehicles and collective entities, and \ac{FMARL} continuously optimises system parameters across multiple levels of abstraction.}
    \label{fig:workflow}
\end{figure}

To illustrate the applicability of the proposed architecture, 
consider the scenario depicted in \Cref{fig:workflow}, 
in which 
a set of autonomous vehicles cooperate to perform highway platooning. 
%
Systems like platooning are inherently complex, 
as they 
require reasoning at multiple levels of abstraction: 
individual vehicles must react to their local perceptions, 
groups of vehicles must coordinate collective manoeuvers, 
and 
the system as a whole must continuously adapt to changing traffic conditions.

Traditional approaches typically 
model cooperation in this kind of system bottom-up,
by explicitly specifying the interactions among vehicles 
and 
the underlying consensus protocols. 
%
In contrast, 
our layered architecture enables the same system to be observed 
and 
controlled at multiple abstraction levels, 
and 
use paradigms that are better suited for different levels.

At the lowest level, 
vehicles self-organise into platoons through \ac{AC} mechanisms, 
which 
allow nearby vehicles satisfying the required conditions 
to identify themselves as members of the same collective entity. 
%
This abstraction 
eliminates the need to explicitly model pairwise interactions 
while 
naturally capturing the dynamic formation and dissolution of platoons.

Once a platoon has emerged, 
the self-organising layer can treat it as a single entity 
and 
specify its collective behaviour. 
%
Rather than 
coordinating every individual vehicle, 
adaptation can be expressed at the platoon level 
through high-level objectives, 
such as 
initiating a lane change, 
merging with another platoon, 
or splitting into smaller groups. 
%
Importantly, 
the formation of platoons is not mandatory: 
when the conditions for creating a platoon are not satisfied, 
the same mechanisms can operate directly on individual vehicles. 
%
This flexibility allows 
the architecture to seamlessly support multiple organisational structures 
and 
different levels of abstraction within the same system.

Finally, 
a \ac{FMARL} layer 
continuously observes the rewards of the system performance 
and 
can learn how to improve parameter used in the behaviour representation. 
%
\ac{MARL} techniques 
can exploit information collected at different abstraction levels
-- individual vehicles, platoons, or even the complete traffic system --
to tune decision-making policies 
and 
improve metrics such as safety, and traffic throughput.

This scenario 
highlights how the proposed framework combines complementary modelling paradigms 
to address different aspects of the same adaptive \ac{MAS}. 
%
\ac{AC} captures the emergence of collective entities, 
\ac{AMAS} mechanisms govern their behaviour, 
and 
\ac{FMARL} continuously improves system performance, 
resulting 
in a unified architecture capable of operating across multiple levels of abstraction.
...
We have argued that 
the growing complexity of cyber-physical and socio-technical systems calls for 
self-adaptive integration at runtime, 
and that 
no single multi-agent paradigm is sufficient on its own. 
%
We proposed a 
three-layer reference architecture 
that 
combines \ac{AC}, \ac{AMAS}, and \ac{FMARL}, 
discussing how each layer compensates for the limitations of the others: 
\ac{AC} supplies scalable collective coordination, 
\ac{AMAS} adds heterogeneity and bottom-up cooperative behaviour, 
and 
\acl{FMARL} covers the situations that experts cannot foresee.

We identified situation characterisation%
--deciding at runtime which paradigm should govern a given situation--%
as the central open challenge that makes the integration effective, 
alongside the further challenges of 
governance, robust federated aggregation, and multi-objective evaluation. 
%
This work contributes to the broader vision of the 
Self-Improving Systems Integration (SISSY) initiative, 
in which integration concerns are addressed at runtime rather than design time, 
allowing systems to autonomously make decisions that previously required human expertise.
%
Future work will 
instantiate the architecture on the different layers of 
synchromodal transportation scenarios, 
formalise the situation-characterisation mechanism as a runtime decision layer 
between 
self-organisation and learned policies, 
and 
study how intervention events can be turned into training scenarios 
that progressively reduce the need for intervention. 