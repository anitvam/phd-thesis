\chapter{Supporting Autonomic Computing via BDI Tooling}
...
\chapter{From Multi-Paradigm Agents to Multi-Paradigm Systems}
...
%
First,
\Cref{chap:jakta} has shown with the \jakta{} prototype
...
while not losing the core abstractions of \ac{BDI} systems.
%
This improves modularity, reuse, and maintainability,
while lowering the entry barrier.
%
Second,
\Cref{chap:concurrency} has investigated how concurrency is managed in \agentspeak{} frameworks,
proposing a dichotomy between \emph{internal} and \emph{external} concurrency.
%
Then,
\Cref{chap:genai} has investigated how \ac{GenAI} can be integrated with \ac{BDI} agents to support plan generation at runtime,
...
%
Finally,
\Cref{chap:hypermedia} has investigated how the paradigm could be integrated with semantic hypermedia and the Web,
...
thus allowing reasoning on any knowledge representation, not only on \ac{OWL} and \ac{RDF}.
...
The contributions of this part have focused on the \emph{individual} agent:
how it is programmed,
how it is executed,
how it acquires new plans,
and how it represents its knowledge.
%
However,
the \acp{CAS} introduced in \Cref{sec:self-organization}
are made of large populations of interacting entities,
whose intended behaviour is often only meaningfully described at the level of the system as a whole.
%
The natural next step is therefore to ask
how next-generation \ac{BDI} agents can take part in,
and benefit from,
such collective settings.

Software modelling has been a fundamental pillar of the \ac{SASO} community since its very beginning~\cite{karsai1999saso},
and two distinct yet complementary modelling perspectives have emerged within it.
%
The \emph{singular perspective}
focuses on individual entities (i.e., devices, agents),
providing rich abstractions for modelling their behaviour;
examples include the \mapek{} loop (see \Cref{sec:autonomic-computing})
and \ac{PRS}-based architectures such as \ac{BDI} (see \Cref{sec:bdi}).
%
Conversely,
the \emph{aggregated perspective}
emphasises the collective behaviours and emergent properties
that arise from the interaction of several entities,
simplifying the abstractions of the single agents;
\ac{AC} (see \Cref{sec:ac}) is a prominent example.
%
While both perspectives offer valuable modelling capabilities,
they are typically applied separately in practice,
favouring either an advanced individual approach or a collective one.
%
We argue instead that individual agents and collective behaviours
are not competing abstractions,
but complementary aspects of the design of complex adaptive systems.

Building on this observation,
this chapter outlines two research directions
of progressively broader scope.
%
\Cref{sec:perspectives-bdi-ac}
proposes a hybrid framework combining \ac{BDI} agents with \ac{AC},
which directly builds on \jakta{} and on the generalised \ac{BDI} engine of \Cref{sec:generalised-bdi-engine}.
%
\Cref{sec:perspectives-sissy}
widens the view to a layered reference architecture
that combines collective coordination,
cooperative self-organisation,
and federated learning.
%
Finally,
\Cref{sec:perspectives-conclusion} draws the concluding remarks.

%==========================================================
\section{Cognitive Agents in the Field: Combining \acs{BDI} Agents and \acl{AC}}
\label{sec:perspectives-bdi-ac}
%==========================================================

The first direction aims at bridging cognitive \ac{BDI} \ac{AOP} and \ac{AC}
into a unified framework that leverages the strengths of both.
%
The integration of \ac{AC} and \ac{AOP} was first explored in~\cite{viroli2017ijaose},
which examined the incorporation of aggregate plans within \ac{AOP} \acp{MAS}.
%
Building on the future directions outlined in that work,
we advance the integration by adopting \ac{BDI} as the abstraction for \ac{AOP},
and we outline a roadmap for its realisation
that leverages existing frameworks for both paradigms.

\subsection{A Hybrid Cognitive-Collective Framework}

Unlike tools that treat individual and collective intelligence as separate paradigms,
the envisioned framework integrates both perspectives,
combining \ac{BDI} for local cognitive reasoning
with \ac{AC} mechanisms for collective coordination.
%
It is built upon three fundamental features.

\paragraph{Cognitive local decision-making}
Individual agents employ the \ac{BDI} cognitive framework (see \Cref{sec:bdi}) to devise locally-relevant decisions.
%
...
When dealing with tasks that exceed the capabilities of individual agents
...
%
In the envisioned framework,
agents can delegate these tasks to aggregate computations.

\paragraph{Bidirectional information flow}
The framework establishes a bidirectional information flow
between local cognitive agents and the aggregate computing layer.
%
On the one hand, agents gather information from the collective computation
and exploit it to enhance their individual reasoning processes.
%
On the other hand,
the collective layer is informed by the individual situated perceptions of participating agents.

Overall,
such a framework would improve the engineering of \ac{SASO} systems
by enabling the development of adaptive systems that can operate effectively
across a wide range of scenarios and scale requirements.

\subsection{Roadmap}

The framework can be realised by leveraging two Kotlin-based frameworks:
\jakta{} for \ac{BDI} agents (see \Cref{chap:jakta})
and Collektive for \ac{AC} (see \Cref{subsec:ac-frameworks}).
%
Both are distributed as open-source libraries and provided as internal \acp{DSL},
thus inheriting Kotlin's type system, paradigm-blending features, and tooling ecosystem.
%
Sharing the same host language,
they can be combined into a single framework
in which cognitive agents invoke collective computations
as a natural extension of their action capabilities.

\paragraph{Decoupling \jakta{} from logic programming}
The main challenge of this integration lies in
the logic-based nature of \ac{BDI} reasoning,
inherited from the \agentspeak{} semantics.
%
As an implementation of \agentspeak{},
\jakta{} currently comes with its own logic reasoning engine,
whereas Collektive operates in terms of computational fields (see \Cref{subsec:ac-model}).
%
Therefore,
the first step is to break the tight coupling between logic constructs and the \jakta{} implementation,
so that its reasoning process can operate on any kind of structured information,
including the outcomes of aggregate computations.
%
This is precisely the goal of the generalised \ac{BDI} engine proposed in \Cref{sec:generalised-bdi-engine}:
the same separation between the reasoning cycle and knowledge representation
that enables agents to reason over semantic hypermedia
would also enable them to reason over computational fields.

\paragraph{Integrating \jakta{} and Collektive}
Once this is achieved,
\jakta{} and Collektive can be integrated,
since \jakta{} agents can invoke aggregate plans and then reason on their outcome.
%
Particular attention will be needed for execution:
the \ac{BDI} reasoning cycle and the repeated rounds of \ac{AC} (see \Cref{subsec:ac-model})
must be scheduled together,
making the concurrency concerns discussed in \Cref{chap:concurrency}
a central design dimension of the hybrid framework.

\paragraph{Validation through simulation}
Finally,
...
\begin{inlinelist}
    ...
    \item it will enable direct simulation of the \ac{DSL} definition of the system,
...
in systems modelled with the proposed framework,
...
%==========================================================
\section{Self-Integrating Systems: Combining Collective, Cooperative, and Learning-Based Coordination}
\label{sec:perspectives-sissy}
%==========================================================

The second direction broadens the scope from two paradigms to a layered architecture.
%
...
%
It continues the longer tradition of
...
%
In the following,
we first position them with respect to self-adaptation,
then propose a three-layer reference architecture combining them,
identify the open challenges it raises,
and illustrate it on a highway platooning scenario.

\subsection{Centralised and Decentralised Self-Adaptation}
\label{sec:perspectives-sissy-background}
...
such as the autonomic managers discussed in \Cref{sec:autonomic-computing},
which monitor the system and its operating context
to decide whether to trigger adaptations,
typically relying on an explicit internal representation
...
thereby
limiting their scalability, flexibility, and robustness~\cite{al2026collaborative}.

On the other hand,
there are self-adaptive systems designed to function in a decentralised fashion,
following the self-organisation principles discussed in \Cref{sec:self-organization}.
%
...
with coordination mechanisms often taking inspiration from
physics~\cite{DBLP:journals/tosem/MameiZ09,viroli2019jlap},
...
%
However,
since their design is driven by the creation of local rules
in a bottom-up fashion,
they may be difficult to modularise and reuse~\cite{DBLP:conf/icse/BozhinoskiB18}.
%
In general,
...
and
it is difficult to predict the global behaviour of a system in the absence of a global view,
...
\acp{MAS} (see \Cref{sec:agents-mas})
have been widely investigated as self-* systems
lying between these two categories.
%
The three paradigms combined in this direction are all \ac{MAS}-based approaches;
their main features and limitations are summarised below.

\paragraph{\acl{AC}}
As discussed in \Cref{subsec:ac-model},
\ac{AC} designs self-organising systems from a global perspective,
but devices are typically assumed to be homogeneous,
which makes it hard to naturally express and support heterogeneous capabilities and behaviours.
%
...
\begin{enumerate*}[label=(\roman*)]
    ...
    and
    \item a sufficiently comprehensive set of execution scenarios
...
which is
a serious obstacle in safety-critical applications~\cite{gronauer2022multi}.

\paragraph{\acl{FL} for \aclp{MAS}}
\ac{FL}
...
%
These properties are attractive for \acp{MAS},
...
\subsection{A Three-Layer Architecture}
\label{sec:perspectives-sissy-architecture}

Each of the approaches discussed above captures only a subset of the complexity inherent to adaptive \acp{MAS}.
%
Indeed,
...
in which the three approaches interact with one another,
in addition to acting directly on the underlying devices,
so that the strengths of each compensate for the limitations of the others.
...
\paragraph{\acl{AC} and the self-organising layer}
\ac{AC}
...
but its homogeneous programming model makes it difficult to
...
%
Crucially,
\ac{AC} can be leveraged to create entities that are treated as single agents at the \ac{AMAS} layer,
...
while \ac{AMAS}-level swarm commands get automatically translated into control signals for each device by \ac{AC} (top-down control).
%
The combination of \ac{AC} and \ac{AMAS} thus enables both local interactions and emergent behaviour:
agents make decisions based solely on local information
while collectively contributing to a shared global behaviour,
and \ac{AC} provides a shared representation of the environment through a spatio-temporal field,
so that agents can indirectly coordinate their actions without requiring explicit communication or centralised control.

\paragraph{The self-organising layer and \acs{FMARL}}
The \ac{FMARL} layer
...
\paragraph{Where cognitive agents fit}
Although the architecture does not prescribe how the agents of the self-organising layer are programmed,
\ac{BDI} agents are natural candidates for it.
%
The expert-designed, interpretable behaviours this layer relies on
closely match \ac{BDI} plans,
which encode procedural knowledge for the situations the designer anticipated.
%
Moreover, 
the hybrid framework of \Cref{sec:perspectives-bdi-ac}
provides the programming support for the interaction between the \ac{AC} layer and the agents above it.

\subsection{Open Research Challenges}
\label{sec:perspectives-sissy-challenges}
...
\paragraph{Situation characterisation}
...
%
For \ac{BDI} agents,
this decision resembles the evaluation of plan applicability against the current beliefs,
suggesting that the agent's own reasoning cycle could host it.
...
assessing self-improving \acp{MAS} against
...
\subsection{Illustrative Scenario: Highway Platooning}
\label{sec:perspectives-sissy-scenario}
...
groups of vehicles must coordinate collective manoeuvres,
and
...
%
In contrast,
the layered architecture enables the same system to be observed
and
controlled at multiple abstraction levels,
using the paradigm best suited to each level.
...
and
can learn how to improve the parameters used in the behaviour representation.
%
\ac{MARL} techniques
...
\label{sec:perspectives-conclusion}
%==========================================================

This part of the thesis has argued that the limited adoption of \ac{BDI} agents
is not due to their abstractions,
but to the lack of tools that integrate them with mainstream software engineering practices and with other paradigms.
%
The contributions presented so far have addressed this gap at the level of the individual agent,
making \ac{BDI} programmable within a mainstream language,
analysing how it is executed,
enriching it with generative capabilities,
and decoupling its reasoning cycle from any specific knowledge representation.

The perspectives outlined in this chapter extend the same multi-paradigm philosophy
from the single agent to the collective.
%
In the first direction,
\ac{BDI} agents and \ac{AC} are combined so that cognitive agents can rely on collective computations,
and vice versa.
%
In the second,
this combination becomes part of a broader self-integrating architecture,
where collective coordination, cooperative self-organisation, and federated learning
compensate for each other's limitations,
and where deciding at runtime which paradigm should govern a given situation
emerges as the key open challenge.
%
In both cases,
the role envisioned for next-generation \ac{BDI} agents is not that of a self-contained paradigm,
but that of an interoperable cognitive component
within larger multi-paradigm systems.
