...
%
An example of the latter is the \ac{AC} paradigm (see \Cref{sec:ac}).
...
Individual agents employ the \acf{BDI} \ac{AOP} cognitive framework  (see \Cref{sec:bdi}) to devise locally-relevant decisions.
%
With this model, 
...
%
%
Such tasks are the natural target of \ac{AC} (see \Cref{sec:ac}),
which has been successfully applied to domains ranging from crowd management
to swarm-drone coordination (see \Cref{subsec:macroprogramming}).
...
Collektive is a Kotlin framework for \ac{AC} based on \ac{XC},
described in \Cref{subsec:ac-frameworks}.

Both technologies are built on top of Kotlin, distributed as open-source libraries,
and provided as internal \acp{DSL},
thus inheriting Kotlin's type system, paradigm-blending features, and tooling ecosystem
(see \Cref{chap:jakta}).
...
Collektive,
on the other hand,
operates in terms of \emph{computational fields} (see \Cref{subsec:ac-model}).
%
Therefore,
...
we will integrate it with the Alchemist simulator~\cite{PianiniJOS2013}
(see \Cref{subsec:ac-frameworks}).
%
...
\ac{AC} (see \Cref{sec:ac})
designs self-organising systems from a global perspective,
by programming the network of devices as a whole
through functional operators on computational fields,
and by composing reusable, self-stabilising building blocks.

%% AC LIMITATIONS >>
As discussed in \Cref{subsec:ac-model},
the main drawback of the paradigm is that
devices are typically assumed to be homogeneous,
which makes it hard to naturally express and support heterogeneous capabilities and behaviours within \ac{AC}.
%
