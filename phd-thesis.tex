...
recent work extends the language to address new \emph{software engineering} challenges, improve agent \emph{organization}, increase \emph{explainability}, and integrate modern \emph{artificial intelligence} techniques. 
...
The symbolic nature of AgentSpeak makes it particularly suitable for explainable artificial intelligence. Since the internal state of an agent is explicitly represented through beliefs, goals, intentions, and plans, the reasoning process can be inspected and reconstructed more easily than in purely subsymbolic approaches.

Recent work therefore investigates mechanisms for generating human-understandable explanations of agent behaviour. These include reconstructing execution traces, explaining why particular plans were selected or discarded, justifying decisions according to the current belief base, and supporting explanation-oriented communication among agents and human users. Such capabilities are becoming increasingly important in collaborative and human-centred applications, where transparency, accountability, and trust are fundamental system requirements.

Integration with generative artificial intelligence

The recent success of large language models has opened a new research direction for AgentSpeak-based systems. Traditionally, an agent's plan library is statically defined by the developer before execution. More recently, researchers have investigated the possibility of employing generative models to synthesize plans dynamically whenever no existing plan is suitable for the current situation.

In these hybrid architectures, plans generated by a language model can be incorporated into the agent's plan library and subsequently executed according to the standard AgentSpeak operational semantics. The objective is to combine the adaptability and generative capabilities of foundation models with the symbolic reasoning, explicit knowledge representation, and execution transparency offered by the \ac{BDI} architecture. At the same time, this research direction raises several challenges, including the verification of generated plans, their consistency with the agent's belief base, and the preservation of the formal guarantees provided by symbolic reasoning.

Open challenges

Despite nearly three decades of research, several challenges remain open. Current AgentSpeak technologies still exhibit limitations with respect to interoperability, modular software development, concurrency management, debugging support, and static verification. Moreover, while recent advances in explainability and generative artificial intelligence significantly extend the applicability of the language, integrating these capabilities without compromising the formal semantics and predictability of the \ac{BDI} reasoning cycle remains an active area of investigation.

Overall, recent research demonstrates that AgentSpeak(L) is far from being a mature and static language. Instead, it continues to evolve through extensions that preserve its original symbolic and declarative foundations while expanding its applicability to increasingly complex and heterogeneous software systems. These developments also motivate the emergence of new AgentSpeak-inspired technologies, which seek to combine the well-established cognitive model of the language with modern software engineering principles and contemporary artificial intelligence techniques.

\subsection{\agentspeak{} Frameworks}
Programming languages inspired by the \agentspeak{} philosophy, 
preserve its original abstractions and operational semantics,
but differ in some implementation details.

The following subsection reviews the main AgentSpeak-inspired technologies and compares them according to a number of software engineering characteristics relevant to the design and implementation of \acp{MAS}.
